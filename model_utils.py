...
def train_ESRGAN_iter(gen, disc, dataloaders, gen_opt, disc_opt, criterion, global_info, log_dict):
    ...
    for batch_idx, data in enumerate(train_dataloader):
        ...
        if batch_idx % global_info['gen_k'] == 0:
            ...
            log_dict['pixel_loss'].update(pixel_loss.item(), len(inputs))
            ...
            log_dict['gan_loss'].update(gan_loss.item(), len(inputs))
            percep_loss = criterion['vgg_loss'](fake, labels)
            log_dict['percep_loss'].update(percep_loss.item(), len(inputs))
        ...
        if batch_idx % global_info['disc_k'] == 0:
            ...
            log_dict['D_losses_real'].update(disc_loss_real.item(), len(inputs))
            disc_loss_real.backward()

            disc_fake = disc(fake.detach())
            disc_loss_fake = 0.5 * criterion['bce'](disc_fake - disc_real.detach().mean(), torch.zeros_like(disc_fake))
            log_dict['D_losses_fake'].update(disc_loss_fake.item(), len(inputs))
            disc_loss_fake.backward()
            disc_opt.step()
            log_dict['D_losses'].update(disc_loss_fake.item()+disc_loss_real.item(), len(inputs))

            log_dict['F_prob'].update(disc_fake.detach().mean(), len(inputs))
            log_dict['R_prob'].update(disc_real.detach().mean(), len(inputs))
        ...
        if (batch_idx + 1) % global_info['iter_of_epoch'] == 0:
            ...
            if isinstance(gen, torch.nn.DataParallel):
                ...
                disc2 = copy.deepcopy(disc)
            # validation and log
            global_info['tb_writer']['scalar'].add_scalar('DLoss_real', log_dict['D_losses_real'].avg, global_info['step'])
            global_info['tb_writer']['scalar'].add_scalar('DLoss_fake', log_dict['D_losses_fake'].avg, global_info['step'])
            global_info['tb_writer']['scalar'].add_scalar('pixel_loss', log_dict['pixel_loss'].avg, global_info['step'])
            global_info['tb_writer']['scalar'].add_scalar('gan_loss', log_dict['gan_loss'].avg, global_info['step'])
            global_info['tb_writer']['scalar'].add_scalar('percep_loss', log_dict['percep_loss'].avg, global_info['step'])
            global_info.update({'Gloss': log_dict['G_losses'].avg, 'Dloss': log_dict['D_losses'].avg})
            val_SRGAN(gen2, val_dataloader, global_info)
            # save
            save_GAN_checkpoint_iter(gen2, gen_opt, disc2, disc_opt, global_info)
            # update for next iteration
            log_dict.update({'D_losses': utils.AverageMeter(), 'G_losses': utils.AverageMeter(),
                             'D_losses_real': utils.AverageMeter(), 'D_losses_fake': utils.AverageMeter(),
                             'pixel_loss': utils.AverageMeter(), 'gan_loss': utils.AverageMeter(), 'percep_loss': utils.AverageMeter(),
                        'F_prob': utils.AverageMeter(), 'R_prob': utils.AverageMeter()})
            global_info['t'].set_description('step:{}/{}'.format(step, global_info['num_steps'] - 1), refresh=True)
