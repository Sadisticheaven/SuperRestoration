...
def train_model(config, pre_train, from_pth=False):
    ...
    if not from_pth:
        disc.init_weight()
    criterion = {'bce': nn.BCEWithLogitsLoss().to(device), 'pixel_loss': nn.L1Loss().to(device), 'vgg_loss': VGGLoss(device)}
    ...
    start_step, best_step, best_niqe, writer, csv_file = \
        model_utils.load_GAN_checkpoint_iter(pre_train, config['weight_file'], gen, gen_opt,
                                             disc, disc_opt, csv_file, from_pth)
    ...
    global_info = {'device': device, 'step': start_step, 't': None, 'auto_lr': config['auto_lr'], 'milestone': config['milestone'],
                   ...
                   'best_step': best_step, 'iter_of_epoch': iter_of_epoch, 'best_niqe': best_niqe,
                   'pixel_weight': config['pixel_weight'], 'adversarial_weight': config['adversarial_weight'],
                   'disc_k': config['gen_k'], 'gen_k': config['disc_k']}
    log_dict = {'D_losses': utils.AverageMeter(), 'G_losses': utils.AverageMeter(),
                'D_losses_real': utils.AverageMeter(), 'D_losses_fake': utils.AverageMeter(),
                'pixel_loss': utils.AverageMeter(), 'gan_loss': utils.AverageMeter(),
                'percep_loss': utils.AverageMeter(),
